...
def candidates_all():
    ...
    rows_html_list = []
    for r in rows:
        cv = f"<a href='{url_for('download_cv', path=r['cv_path'])}'>CV</a>" if r['cv_path'] else "-"
        actions_html = "-"
        role = current_user()['role']
        if role in (ROLE_MANAGER, ROLE_ADMIN):
            actions_html = (
                f"<a class='btn light' href='{url_for('assign_candidate', candidate_id=r['id'])}'>Assign</a> "
                f"<a class='btn' href='{url_for('finalize_candidate', candidate_id=r['id'])}'>Finalize</a>"
            )
        elif role == ROLE_INTERVIEWER and r['interviewer_id'] == current_user()['id']:
            actions_html = f"<a class='btn' href='{url_for('interview_feedback', candidate_id=r['id'])}'>Feedback</a>"
